...
1. Capture treesitter patterns in after/queries/highlight.scm:
        ex. (identifier) @custom_capture or @variable.builtin
2. Link the capture_id to a nightfox group or your own custom group:
        ex. vim.api.nvim_set_hl(0, 'myCustomCapture.swift', {})
        ex. vim.api.nvim_set_hl(0, 'myCustomCapture.swift', { link = 'MyCustomGroup' })

        Note: You can also link standard highlight-groups but put it into an autocommand
        because colors.lua seems to get sourced earlier...
        ex. vim.api.nvim_create_autocmd('VimEnter', {
                callback = function()
                    vim.api.nvim_set_hl(0, 'Comment', { link = 'MyCustomGroup' })
                end
            }) 
3. Set dayfox's (or whatever theme's) syntax. You can create custom syntax and palette fields as well:
        ex. palettes = {
                dayfox = {
                    ...
                    magenta = '#FFFFFF'
...
4. Build the highlight-group using a theme's syntax value, palette. Note that highlight groups are NOT differentiated by theme
   hence all = {...}. Also note that the styles option is NOT a template source. So you can't do style = 'dayfox.comment' or
   style = 'comment':
        ex. groups = {
                all = {
...
-- Still keep empty hl groups cus if you switch from dayfox to any other theme, groups (below) get activated.
-- Setting foreground using `fg = palettes.dayfox...` is specific to DAYFOX.
